    ...
    {anchors.map(anchor => {
      ...
      return <section key={anchor.id} className="rounded-xl border border-j-border bg-white p-3 shadow-sm">
        <div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-j-border pb-1.5">
          <h3 className="text-sm font-bold text-j-ink">{anchor.label}</h3>
          <span className="text-2xs font-bold uppercase tracking-wide text-j-ink-faint">{anchor.surface} · {rows.length} block{rows.length === 1 ? "" : "s"}</span>
        </div>
        ...
        {rows.map((block, index) => {
          ...
          return <div key={block.id} className="grid gap-2 border-b border-j-border py-2 last:border-b-0 sm:grid-cols-[minmax(0,1fr)_6rem_auto]">
            ...
            <div className="space-y-1.5">
              ...
              <label className="flex items-center gap-1.5 text-xs font-medium text-j-ink-soft">
            ...
            <div className="flex items-start gap-1">
              ...
              <button type="button" disabled={busy || !dirty} onClick={() => void run(() => update.mutateAsync({ id: block.id, anchorId: block.anchorId, heading: value.heading, body: value.body, tone: value.tone, active: value.active }))} className="h-7 rounded-lg bg-j-accent px-2 text-xs font-bold text-white disabled:opacity-30">Save</button>
        ...
        <button type="button" disabled={busy} onClick={() => void run(() => create.mutateAsync({ anchorId: anchor.id, heading: "New notice", body: "", tone: "info", active: false }))} className="mt-2 inline-flex h-7 items-center gap-1 rounded-lg border border-j-border px-2 text-xs font-bold text-j-accent disabled:opacity-40">
