...
function OwnerReportContent() {
  ...
  return <AdminWorkspaceLayout title="Owner activity report">
    <main className="mx-auto w-full max-w-7xl space-y-5 pb-10">
      ...
      {report.isLoading ? <div className="flex min-h-[34vh] items-center justify-center text-j-ink-soft"><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Preparing the activity report…</div> : report.isError || !report.data ? <section className="rounded-3xl border border-red-200 bg-red-50 p-6 text-sm text-red-800">The Owner activity report could not be loaded. Please refresh and try again.</section> : <ReportData data={report.data as OwnerActivityReport} />}
    </main>
