...
export function GuardianOpenRequestsPanel({
  ...
  return <Card role="region" aria-label="Open requests" className="rounded-3xl border-j-border shadow-sm"><CardHeader><CardTitle className="text-xl font-black text-j-ink">Open requests</CardTitle></CardHeader><CardContent className="space-y-3 p-6 pt-0">
...
function GuardianHowItWorksPanel() {
  ...
  return <div className="space-y-6"><h1 className="text-3xl font-black tracking-tight text-j-ink">How it works</h1><Card className="rounded-3xl border-j-border shadow-sm"><CardContent className="divide-y divide-j-border p-7">{steps.map(([number, title, detail]) => <div key={number} className="flex gap-4 py-5 first:pt-0 last:pb-0"><div className="grid size-9 shrink-0 place-items-center rounded-xl bg-blue-50 text-sm font-black text-[#1677c8]">{number}</div><div><h2 className="font-extrabold text-j-ink">{title}</h2><p className="mt-1 text-sm leading-6 text-j-ink-soft">{detail}</p></div></div>)}</CardContent></Card><Card className="rounded-3xl border-sky-100 bg-sky-50 shadow-sm"><CardContent className="p-6"><p className="font-extrabold text-sky-950">Need help with a request?</p><a href={contact.whatsapp()} target="_blank" rel="noreferrer" className="mt-4 inline-flex rounded-xl bg-[#1677c8] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#0e4f85]">Contact support on WhatsApp</a></CardContent></Card></div>;
}
...
export function GuardianDashboardContent({ section, requestId }: { section?: string; requestId?: number }) {
  ...
  if (section && section !== "hire" && section !== "profile" && section !== "posted-jobs" && section !== "notifications" && section !== "confirmation-letter" && section !== "attendance" && section !== "settings" && section !== "how-it-works") {
    return <div className="space-y-6"><GuardianWorkspaceState kind="planned" title={`${sectionLabel} is coming soon`} message="This Guardian workspace section is planned and will be introduced after its data and privacy rules are ready." /><Card className="rounded-3xl border-j-border shadow-sm"><CardContent className="flex flex-col items-start gap-4 p-7 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-extrabold text-j-ink">Need help now?</p></div><div className="flex flex-wrap gap-3"><Link href="/guardian/dashboard/hire"><Button className="bg-[#1677c8] hover:bg-[#0e4f85]">Hire a tutor</Button></Link><Link href="/guardian/requests"><Button variant="outline">View requests</Button></Link></div></CardContent></Card></div>;
  }
