...
export function GuardianOpenRequestsPanel({
  ...
  return <Card role="region" aria-label="Open requests" className="rounded-3xl border-slate-200 shadow-sm"><CardHeader><CardTitle className="text-xl font-black text-slate-950">Open requests</CardTitle></CardHeader><CardContent className="space-y-3 p-6 pt-0">
    ...
    {!isLoading && !hasError && openRequests.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5"><p className="font-extrabold text-slate-950">No open requests yet</p><Link href="/guardian/dashboard/hire" className="mt-4 inline-flex rounded-xl bg-[#1677c8] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#0e4f85] focus:outline-none focus:ring-2 focus:ring-[#1677c8]">Hire a tutor</Link></div> : null}
...
function GuardianConfirmationLetterPanel() {
  ...
  return <div className="space-y-6"><h1 className="text-3xl font-black tracking-tight text-slate-950">Confirmation letter</h1><Card className="rounded-3xl border-slate-200 shadow-sm"><CardContent className="p-0">{lettersQuery.isLoading ? <div className="p-7 text-sm text-slate-600">Loading your private confirmation letters…</div> : null}{lettersQuery.error ? <div className="p-7"><GuardianWorkspaceState kind="error" title="Confirmation letters are temporarily unavailable" message="Please try again. Your private request information remains protected." onRetry={() => { void lettersQuery.refetch(); }} /></div> : null}{!lettersQuery.isLoading && !lettersQuery.error && letters.length === 0 ? <div className="p-8 text-center"><ShieldCheck className="mx-auto size-8 text-[#1677c8]" /><h2 className="mt-4 text-lg font-black text-slate-950">No issued letter yet</h2><Link href="/guardian/dashboard/posted-jobs" className="mt-5 inline-flex"><Button variant="outline">Review posted jobs</Button></Link></div> : null}{!lettersQuery.isLoading && !lettersQuery.error && letters.length > 0 ? <div className="divide-y divide-slate-100">{letters.map(letter => <div key={letter.id} className="p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-extrabold text-slate-950">Letter {letter.letterNumber}</p><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${letter.status === "issued" ? "bg-emerald-50 text-emerald-800" : "bg-slate-100 text-slate-700"}`}>{letter.status === "issued" ? "Issued" : "Superseded"}</span><span className="text-xs font-semibold text-slate-500">Version {letter.version}</span></div><p className="mt-2 text-sm leading-6 text-slate-600">Issued {formatGuardianDate(letter.issuedAt)}. This bilingual document confirms the approved tutor-match schedule.</p>{letter.supersededAt ? <p className="mt-2 text-xs font-semibold text-amber-800">A later request change superseded this record on {formatGuardianDate(letter.supersededAt)}.</p> : null}</div>{letter.status === "issued" ? <GuardianConfirmationLetterDownloadButton letterId={letter.id} /> : null}</div></div>)}</div> : null}</CardContent></Card></div>;
}
