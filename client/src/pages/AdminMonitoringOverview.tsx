...
function OverviewContent() {
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-5 pb-10">
    ...
    <section className="grid gap-5 lg:grid-cols-2"><article className="rounded-3xl border border-j-border bg-white p-5 shadow-sm sm:p-6"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-bold text-j-ink">Tutor moderation queue</h2></div><UserRoundCog className="h-6 w-6 text-j-accent" /></div><div className="mt-5 flex items-center justify-between rounded-2xl bg-j-surface-sunken p-4"><div><p className="text-xs font-bold uppercase tracking-wide text-j-ink-muted">Suspended profiles</p><p className="mt-1 text-xl font-bold text-j-ink">{metrics.suspendedTutors}</p></div><Link href="/admin/tutors" className="inline-flex items-center gap-1 rounded-xl bg-j-accent px-3 py-2 text-sm font-bold text-white hover:bg-j-accent-hover">Manage Tutors <ArrowRight size={15} /></Link></div></article><article className="rounded-3xl border border-j-border bg-white p-5 shadow-sm sm:p-6"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-bold text-j-ink">Guardian request activity</h2><p className="mt-1 text-sm leading-6 text-j-ink-soft">Use request context for coordination. Contact detail access is deliberate and is recorded in an append-only audit trail.</p></div><ContactRound className="h-6 w-6 text-j-accent" /></div><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-j-surface-sunken p-4"><p className="text-xs font-bold uppercase tracking-wide text-j-ink-muted">Reviewing</p><p className="mt-1 text-xl font-bold text-j-ink">{metrics.reviewingRequests}</p></div><div className="rounded-2xl bg-j-surface-sunken p-4"><p className="text-xs font-bold uppercase tracking-wide text-j-ink-muted">Matched</p><p className="mt-1 text-xl font-bold text-j-ink">{metrics.matchedRequests}</p></div></div><Link href="/admin/guardians" className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-j-accent hover:text-[#0d5da4]">Open Guardian activity <ArrowRight size={15} /></Link></article></section>
    <section className="rounded-3xl border border-j-border bg-white p-5 shadow-sm sm:p-6"><h2 className="text-lg font-bold text-j-ink">Accountability activity</h2><div className="mt-5 grid gap-3 md:grid-cols-2"><div className="rounded-2xl border border-j-border p-4"><p className="text-xs font-bold uppercase tracking-wide text-j-ink-muted">Recent Tutor moderation</p>{overview.data.recentModeration.length ? <ul className="mt-3 space-y-2 text-sm text-j-ink-soft">{overview.data.recentModeration.map(event => <li key={event.id} className="flex justify-between gap-3"><span>Tutor {event.tutorId} → <strong className="capitalize">{event.nextStatus.replaceAll("_", " ")}</strong></span><time className="shrink-0 text-xs text-j-ink-muted">{new Date(event.createdAt).toLocaleString()}</time></li>)}</ul> : <p className="mt-3 text-sm text-j-ink-muted">No Tutor moderation events yet.</p>}</div><div className="rounded-2xl border border-j-border p-4"><p className="text-xs font-bold uppercase tracking-wide text-j-ink-muted">Recent Guardian contact access</p>{overview.data.recentContactAccess.length ? <ul className="mt-3 space-y-2 text-sm text-j-ink-soft">{overview.data.recentContactAccess.map(event => <li key={event.id} className="flex justify-between gap-3"><span>Request #{event.tutorRequestId}</span><time className="shrink-0 text-xs text-j-ink-muted">{new Date(event.createdAt).toLocaleString()}</time></li>)}</ul> : <p className="mt-3 text-sm text-j-ink-muted">No Guardian contact disclosures yet.</p>}</div></div></section>
  </div>;
}
