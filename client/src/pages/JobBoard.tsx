...
function JobBoardFilters({ filters, update, cities, locations }: { filters: JobBoardFilterState; update: <K extends keyof JobBoardFilterState>(key: K, value: JobBoardFilterState[K]) => void; cities: Array<{ id: string; label: string }>; locations: Array<{ id: string; label: string }> }) {
  return <div className="space-y-4">
    ...
    <p className="rounded-xl bg-[#f5faff] p-3 text-xs leading-5 text-[#52748d]"><ShieldCheck className="mr-1 inline h-3.5 w-3.5 text-[#167ddd]" />Locations are shown at area level only. Exact family addresses are coordinated privately.</p>
  </div>;
}
...
function ApplicationControl({ interest, isTutor, isApprovedTutor, isInterestSaving, onAction }: { interest?: TutorJobInterest; isTutor: boolean; isApprovedTutor: boolean; isInterestSaving: boolean; onAction: () => void }) {
  ...
  return <div className="mt-3 rounded-xl border border-[#cfe8f7] bg-[#f4fbff] p-3"><div className="flex items-start gap-2"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-[#167ddd]" /><p className="text-xs leading-5 text-[#55738a]">{copy.description}</p></div><button type="button" onClick={onAction} className="motion-interactive mt-3 inline-flex min-h-10 w-full items-center justify-center rounded-xl bg-[#167ddd] px-3 text-sm font-bold text-white hover:bg-[#0b6db0]">{copy.label}</button></div>;
}
...
function JobDetails({ job, onClose, interest, isTutor, isApprovedTutor, isInterestSaving, onInterestAction }: { job: JobBoardJob; onClose: () => void; interest?: TutorJobInterest; isTutor: boolean; isApprovedTutor: boolean; isInterestSaving: boolean; onInterestAction: () => void }) {
  ...
  return <SharedJobDetailsModal
    ...
    onClose={onClose}
    action={<>
      {reason ? <span className="mr-auto text-2xs text-[#6c879e]">{reason}</span> : null}
