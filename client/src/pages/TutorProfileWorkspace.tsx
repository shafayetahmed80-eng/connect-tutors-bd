...
function TutorProfileWorkspaceBody({
  ...
  /**
   * The label to draw for a field: the Owner's rename when one is stored,
   * otherwise the wording passed in. An override is in effect only when the
   * resolved label differs from the registry default, so an untouched field
   * keeps its exact current copy - "(Optional)" suffixes included.
   */
  const fieldLabel = (fieldId: string, fallback: string): string => {
    const meta = findTutorProfileFieldMeta(fieldId);
    const resolved = fieldConfig.byId.get(fieldId);
    return meta && resolved && resolved.label !== meta.label ? resolved.label : fallback;
  };
  ...
  const renderField = (fieldId: string): React.ReactNode => {
    switch (fieldId) {
      ...
      case "privateDetails.additionalPhone": return <FormPhoneInput label={fieldLabel(fieldId, "Additional Phone (Optional)")} value={form.privateDetails.additionalPhone} onChange={value => updatePrivateDetail("additionalPhone", value)} error={phoneErrors.additionalPhone} />;
      case "privateDetails.socialProfileLinks": return <FormInput label={fieldLabel(fieldId, "Social Profile Links (Optional)")} placeholder="Ex- https://facebook.com/username" value={form.privateDetails.socialProfileLinks} onChange={event => updatePrivateDetail("socialProfileLinks", event.target.value)} />;

      case "privateDetails.fatherName": return <FormInput label={fieldLabel(fieldId, "Father’s Name")} placeholder="Ex- Abdul Karim" required value={form.privateDetails.fatherName} onChange={event => updatePrivateDetail("fatherName", event.target.value)} />;
      case "privateDetails.fatherPhone": return <FormPhoneInput label={fieldLabel(fieldId, "Father’s Phone Number")} required value={form.privateDetails.fatherPhone} onChange={value => updatePrivateDetail("fatherPhone", value)} error={phoneErrors.fatherPhone} />;
      case "privateDetails.motherName": return <FormInput label={fieldLabel(fieldId, "Mother’s Name (Optional)")} placeholder="Ex- Abdul Karim" value={form.privateDetails.motherName} onChange={event => updatePrivateDetail("motherName", event.target.value)} />;
      case "privateDetails.motherPhone": return <FormPhoneInput label={fieldLabel(fieldId, "Mother’s Phone Number (Optional)")} value={form.privateDetails.motherPhone} onChange={value => updatePrivateDetail("motherPhone", value)} error={phoneErrors.motherPhone} />;
      case "privateDetails.emergencyContactName": return <FormInput label={fieldLabel(fieldId, "Emergency Contact Name (Optional)")} placeholder="Ex- Rahima Begum" value={form.privateDetails.emergencyContactName} onChange={event => updatePrivateDetail("emergencyContactName", event.target.value)} />;
      case "privateDetails.emergencyContactRelation": return <FormInput label={fieldLabel(fieldId, "Emergency Contact Relation (Optional)")} placeholder="Ex- Uncle" value={form.privateDetails.emergencyContactRelation} onChange={event => updatePrivateDetail("emergencyContactRelation", event.target.value)} />;
      case "privateDetails.emergencyContactPhone": return <FormPhoneInput label={fieldLabel(fieldId, "Emergency Contact Phone (Optional)")} value={form.privateDetails.emergencyContactPhone} onChange={value => updatePrivateDetail("emergencyContactPhone", value)} error={phoneErrors.emergencyContactPhone} />;
      case "privateDetails.emergencyContactAddress": return <FormTextArea label={fieldLabel(fieldId, "Emergency Contact Address (Optional)")} rows={2} value={form.privateDetails.emergencyContactAddress} onChange={event => updatePrivateDetail("emergencyContactAddress", event.target.value)} />;
      ...
      case "resultGpa": return <FormInput label={fieldLabel(fieldId, `${tutorProfileCopy.fields.resultGpa} (Optional)`)} value={form.resultGpa} onChange={event => update("resultGpa", event.target.value)} placeholder="Ex-4.00" />;
      case "deptId": return <FormInput label={fieldLabel(fieldId, `${tutorProfileCopy.fields.deptId} (Optional)`)} value={form.deptId} onChange={event => update("deptId", event.target.value)} placeholder="Ex-13104096" />;
      ...
      case "yearSemester": return form.studyStatus === "studying"
        ? <FormInput label={fieldLabel(fieldId, tutorProfileCopy.fields.yearSemester)} showRequiredMarker value={form.yearSemester} onChange={event => update("yearSemester", event.target.value)} placeholder="Ex- 2nd Year/Semester" error={fieldErrors.yearSemester} />
        : null;
      case "graduationYear": return form.studyStatus === "studying"
        ? null
        : <FormInput label={fieldLabel(fieldId, tutorProfileCopy.fields.graduationYear)} showRequiredMarker inputMode="numeric" maxLength={4} value={form.graduationYear} onChange={event => update("graduationYear", digitsOnly(event.target.value, 4))} placeholder="Ex-2022" error={fieldErrors.graduationYear} />;

      case "universityIdDocumentStatus": return <DocumentUploadRow
        inputId="tutor-university-id-document"
        label={fieldLabel(fieldId, "University ID card")}
      ...
      case "primarySubjectIds": return <SearchableMultiSelect label={fieldLabel(fieldId, tutorProfileCopy.fields.primarySubjects)} required options={toSelectorOptions(subjects.data)} selectedIds={form.primarySubjectIds} onChange={value => update("primarySubjectIds", value)} emptyMessage="No subjects found." error={fieldErrors.primarySubjectIds} maxSelections={Math.max(1, limits["tutor.subjects"] - form.additionalSubjectIds.length)} />;
      case "additionalSubjectIds": return <SearchableMultiSelect label={fieldLabel(fieldId, tutorProfileCopy.fields.additionalSubjects)} options={toSelectorOptions(subjects.data)} selectedIds={form.additionalSubjectIds} onChange={value => update("additionalSubjectIds", value)} emptyMessage="No subjects found." maxSelections={Math.max(0, limits["tutor.subjects"] - form.primarySubjectIds.length)} />;
      case "classLevelIds": return <SearchableMultiSelect label={fieldLabel(fieldId, tutorProfileCopy.fields.classLevels)} required options={groupedClassLevels.options} selectedIds={groupedClassLevels.selectedIds} onChange={value => update("classLevelIds", expandGroupedClassLevelIds(value, groupedClassLevels.groupedIds))} emptyMessage="No classes or levels found." error={fieldErrors.classLevelIds} maxSelections={limits["tutor.levels"]} />;
      case "curriculumIds": return <SearchableMultiSelect label={fieldLabel(fieldId, tutorProfileCopy.fields.curricula)} required options={toSelectorOptions(curricula.data)} selectedIds={form.curriculumIds} onChange={value => update("curriculumIds", value)} emptyMessage="No curricula found." error={fieldErrors.curriculumIds} />;
      case "teachingExperienceYears": return <FormInput label={fieldLabel(fieldId, tutorProfileCopy.fields.teachingExperience)} showRequiredMarker type="number" min="0" max="60" value={form.teachingExperienceYears} onChange={event => update("teachingExperienceYears", event.target.value)} error={fieldErrors.teachingExperienceYears} />;
      case "priorTeachingExperience": return <FormTextArea label={fieldLabel(fieldId, "Prior Teaching Experience")} value={form.priorTeachingExperience} onChange={event => update("priorTeachingExperience", event.target.value)} placeholder="Briefly describe previous tutoring, coaching, or classroom experience." />;
      ...
      >
        ...
        <span><strong className="block text-sm font-semibold text-[#244a6a]">{fieldLabel(fieldId, "Available nationwide for online tuition")}{form.tuitionType === "online" || form.tuitionType === "both" ? <span aria-hidden="true" className={tp.requiredMark}> *</span> : null}</strong><span className="mt-0.5 block text-xs leading-5 text-[#72889a]">Required for Online or Both.</span><InlineError message={fieldErrors.availableNationwide} /></span>
      </label>;
      ...
      case "teachingAreaIds": return <SearchableMultiSelect label={fieldLabel(fieldId, tutorProfileCopy.fields.teachingAreas)} required options={teachingAreaOptions} selectedIds={form.teachingAreaIds} onChange={value => update("teachingAreaIds", value)} onSearchQueryChange={setTeachingAreaQuery} emptyMessage="No areas found." error={fieldErrors.teachingAreaIds} />;
      case "feeMin": return <FormInput label={fieldLabel(fieldId, tutorProfileCopy.fields.feeMin)} showRequiredMarker type="number" min="0" max="500000" inputMode="numeric" value={form.feeMin} onChange={event => update("feeMin", event.target.value)} error={fieldErrors.feeMin} />;
      ...
      case "travelDistanceKm": return <FormInput label={fieldLabel(fieldId, "Travel Distance (km) (Optional)")} placeholder="Ex- 5" type="number" min="1" max="100" inputMode="numeric" value={form.travelDistanceKm} onChange={event => update("travelDistanceKm", event.target.value)} />;
      ...
      case "additionalNotes": return <FormTextArea label={fieldLabel(fieldId, "Additional Notes (Optional)")} rows={4} maxLength={2000} value={form.additionalNotes} onChange={event => update("additionalNotes", event.target.value)} placeholder="Non-sensitive information for the review team." hint={`${form.additionalNotes.length}/2000 characters`} />;
      ...
      default:
        if (fieldId.startsWith("supportingDocument.")) {
          ...
          return <DocumentUploadRow
            ...
            label={fieldLabel(fieldId, tutorSupportingDocumentLabels[documentType])}
            uploadLabel="Upload"
