...
function TutorRequests({ requests, isLoading }: { requests: any[]; isLoading: boolean }) {
  ...
  function TutorSettings({ email }: { email: string }) { return <section className="max-w-3xl rounded-xl border border-[#dce8f0] bg-white p-6 shadow-[0_12px_30px_rgba(38,83,117,0.06)]"><div className="flex items-start gap-4"><span className="rounded-xl bg-[#edf8ff] p-3 text-j-accent"><Mail size={22} /></span><div><h2 className="text-lg font-bold text-j-ink">Secure account</h2><p className="mt-2 text-sm leading-6 text-[#647e92]">Your Tutor account uses email/password sign-in. Passwords are stored only as one-way server-side hashes.</p><p className="mt-4 rounded-xl bg-[#f5f9fc] px-4 py-3 text-sm font-semibold text-[#315b78]">{email}</p></div></div><div className="mt-6 border-t border-[#e7eef3] pt-5"><Link href="/tutors" className="inline-flex items-center text-sm font-bold text-j-accent max-md:min-h-10">View public Tutor Directory</Link></div></section>; }
