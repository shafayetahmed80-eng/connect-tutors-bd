...
import { TUTOR_SETTINGS_PATH, TutorSettingsContent } from "@/pages/PanelSettings";
...
function TutorConfirmationLetterPanel() {
  ...
  return <section className="rounded-xl border border-[#dce8f0] bg-white shadow-[0_12px_30px_rgba(38,83,117,0.06)]"><div className="border-b border-j-border p-6 sm:p-7"><p className="text-xs font-bold uppercase tracking-[0.18em] text-j-accent">Private match record</p><h2 className="mt-2 text-xl font-bold text-j-ink">Issued confirmation letters</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#627e94]">Only letters issued for a request assigned to your account appear here. They exclude Guardian addresses, contacts, and private notes.</p></div>{lettersQuery.isLoading ? <p className="p-7 text-sm text-[#627e94]">Loading your private confirmation letters…</p> : null}{lettersQuery.error ? <div className="p-7"><p className="font-bold text-rose-800">Confirmation letters are temporarily unavailable.</p><Button type="button" variant="outline" className="mt-4" onClick={() => { void lettersQuery.refetch(); }}>Try again</Button></div> : null}{!lettersQuery.isLoading && !lettersQuery.error && letters.length === 0 ? <div className="p-8 text-center"><FileCheck2 className="mx-auto text-j-accent" size={34} /><h3 className="mt-4 text-lg font-bold text-j-ink">No issued letter yet</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#627e94]">When an Admin confirms your assigned Tutor match and approves the official letter, it will appear here.</p><Link href="/tutor/dashboard/requests" className="mt-5 inline-flex"><Button variant="outline">View Tutor requests</Button></Link></div> : null}{!lettersQuery.isLoading && !lettersQuery.error && letters.length > 0 ? <div className="divide-y divide-j-border">{letters.map(letter => <div key={letter.id} className="flex flex-col gap-4 p-6 sm:flex-row sm:items-start sm:justify-between sm:p-7"><div><div className="flex flex-wrap items-center gap-2"><p className="font-bold text-j-ink">Letter {letter.letterNumber}</p><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${letter.status === "issued" ? "bg-emerald-50 text-emerald-800" : "bg-j-surface-muted text-j-ink-soft"}`}>{letter.status === "issued" ? "Issued" : "Superseded"}</span><span className="text-xs font-semibold text-[#627e94]">Version {letter.version}</span></div><p className="mt-2 text-sm leading-6 text-[#627e94]">Issued {formatTutorLetterDate(letter.issuedAt)}. This letter confirms the approved tutor-match schedule.</p>{letter.supersededAt ? <p className="mt-2 text-xs font-semibold text-amber-800">This letter was superseded on {formatTutorLetterDate(letter.supersededAt)}.</p> : null}</div>{letter.status === "issued" ? <ConfirmationLetterViewButton letterId={letter.id} letterNumber={letter.letterNumber} /> : null}</div>)}</div> : null}</section>;
}

function Preferences({ profile, navigate }: { profile: any; navigate: (path: string) => void }) { if (!profile) return <EmptyPanel title="Complete your Tutor Profile first" copy="Subjects, class levels, tuition mode, fee, and availability are saved in your Tutor Profile." action="Complete profile" onClick={() => navigate("/tutor/dashboard/profile")} />; return <section className="grid gap-5 md:grid-cols-2"><SummaryCard icon={BookOpenCheck} title="Teaching subjects" value={profile.subjects.join(", ")} /><SummaryCard icon={BriefcaseBusiness} title="Class levels" value={profile.levels.join(", ")} /><SummaryCard icon={MapPin} title="Tuition delivery" value={`${(profile.tuitionTypes ?? []).map((mode: string) => ({ home: "Home", online: "Online", group: "Group", package: "Package" })[mode] ?? mode).join(" & ") || "Not set"} · ${profile.locationLabel}`} /><SummaryCard icon={CircleHelp} title="Fee & availability" value={`BDT ${profile.fee.toLocaleString()} per month · ${profile.availability}`} /><div className="md:col-span-2 rounded-xl border border-[#dce8f0] bg-white p-6"><p className="text-sm leading-6 text-[#617d92]">Need to revise a subject, class level, fee, or availability? Update your full Tutor Profile. The updated profile is submitted for review.</p><Button onClick={() => navigate("/tutor/dashboard/profile")} className="mt-4 rounded-xl bg-j-accent font-bold hover:bg-[#0e6dc2]"><FilePenLine size={16} /> Edit tuition preferences</Button></div></section>; }
function SummaryCard({ icon: Icon, title, value }: { icon: typeof BookOpenCheck; title: string; value: string }) { return <article className="rounded-xl border border-[#dce8f0] bg-white p-6 shadow-[0_10px_25px_rgba(38,83,117,0.05)]"><Icon className="text-j-accent" size={22} /><h2 className="mt-4 text-sm font-bold text-[#244a6a]">{title}</h2><p className="mt-2 text-sm leading-6 text-[#647f93]">{value}</p></article>; }
function TutorRequests({ requests, isLoading }: { requests: any[]; isLoading: boolean }) {
  ...
  if (!requests.length) return <section className="rounded-xl border border-[#dce8f0] bg-white p-6 text-center shadow-[0_12px_30px_rgba(38,83,117,0.06)] sm:p-10"><ClipboardList className="mx-auto text-j-accent" size={36} /><h2 className="mt-4 text-xl font-bold text-j-ink">No assigned requests yet</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-[#688297]">When an Admin finds a suitable request for your approved Tutor Profile, its learning requirements will appear here. Guardian contact details remain private during matching.</p></section>;
