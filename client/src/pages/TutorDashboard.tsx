...
import { BadgeCheck, BookOpenCheck, BriefcaseBusiness, CircleHelp, ClipboardList, CreditCard, FileCheck2, FilePenLine, GraduationCap, HeartHandshake, LayoutDashboard, LogOut, Mail, MapPin, Settings, Share2, Sparkles, UserRound, UsersRound } from "lucide-react";
...
function TutorSidebarIdentity({ identity }: { identity: ReturnType<typeof getTutorSidebarIdentity> }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl bg-[#f7fbfe] p-2.5 ring-1 ring-[#dcebf5] group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:rounded-xl group-data-[collapsible=icon]:p-1.5">
      ...
      <div className="min-w-0 flex-1 group-data-[collapsible=icon]:hidden">
        ...
        <div className="mt-2 flex items-center justify-between gap-2 border-t border-[#dcebf5] pt-2 text-[10px] font-semibold text-[#6c8799]">
          <span className="truncate">{identity.tutorNumber}</span>
          <span className="shrink-0">{identity.joined}</span>
        </div>
      </div>
    </div>
  );
...
export default function TutorDashboard() {
  ...
  return <DashboardLayout navigationItems={tutorDashboardNavigation} title="Tutor Portal" loginPath="/tutor/login" signOutPath="/tutor/login" onBeforeNavigation={confirmProfileNavigation} sidebarIdentity={<TutorSidebarIdentity identity={sidebarIdentity} />} tutorWorkspaceHeader={sidebarIdentity} onTutorSignOutSuccess={markCurrentTutorSignedOutNotice}>
