...
const button = "inline-flex min-h-12 items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-extrabold transition focus:outline-none focus:ring-4 focus:ring-[#167ddd]/25 disabled:cursor-wait disabled:opacity-60";
const stageSteps = [
...
export function getGuardianRequestJourneyPresentation({ embedded }: { embedded: boolean }) {
  return embedded
    ? { showPublicChrome: false, rootClassName: "bg-transparent text-[#173b60]" }
    : { showPublicChrome: true, rootClassName: "site-page min-h-screen bg-[#f5f8ff] text-[#173b60]" };
}
...
export default function GuardianRequestJourney({ embedded = false }: { embedded?: boolean }) {
  ...
  return <div className={presentation.rootClassName}>
    ...
    <main className={embedded ? "py-0" : "px-4 py-10 sm:px-6 lg:py-16"}><div className={embedded ? "max-w-none" : "mx-auto max-w-6xl"}><div className="grid gap-7 lg:grid-cols-[.8fr_1.2fr] lg:items-start">
      <aside className="rounded-[2rem] bg-[#0d5d9d] p-7 text-white shadow-[0_18px_46px_rgba(13,93,157,.2)] sm:p-10">
        ...
        <div className="mt-8 space-y-4 rounded-2xl border border-white/15 bg-[#0b518a] p-5 text-sm leading-6 text-[#e0f4ff]">
      ...
      <section className="rounded-[2rem] border border-[#dcecf5] bg-white p-5 shadow-[0_18px_55px_rgba(28,101,148,.1)] sm:p-9">
        <JourneyProgress activeStep={stageProgress} ariaLabel="Tutor request journey progress" steps={stageSteps} />
        {stage !== "success" && journeyError ? <p role="alert" className="mt-6 rounded-xl border border-[#f2c4c4] bg-[#fff5f5] px-4 py-3 text-sm font-semibold leading-6 text-[#a43232]">{journeyError}</p> : null}
        ...
        {stage === "request" && isEditMode && (authQuery.isLoading || guardianRequestsQuery.isLoading || loadedEditRequestId !== editRequestId) ? <div className="mt-8 rounded-2xl border border-[#dcecf5] bg-[#f8fcff] p-6 text-center text-sm font-semibold text-[#315b79]"><Loader2 className="mx-auto mb-3 animate-spin text-[#167ddd]" size={22} />Loading your private Pending request securely…</div> : null}
...
function PhoneStage({ phone, onPhoneChange, pending, onContinue }: { phone: string; onPhoneChange: (value: string) => void; pending: boolean; onContinue: () => void }) {
  return <div className="mt-8"><Eyebrow step="Step 1 of 3" title="Start with your phone number" copy="We do not use OTP here. Your number supports a secure handoff if we match you with a Tutor." /><label className="mt-8 block text-sm font-extrabold text-[#315b79]" htmlFor="guardian-phone">Bangladesh mobile number <span className="text-[#d74545]">*</span><span className="mt-2 flex rounded-xl border border-[#cfe2ed] bg-[#f8fcff] focus-within:border-[#167ddd] focus-within:ring-4 focus-within:ring-[#167ddd]/10"><span className="border-r border-[#dcecf5] px-4 py-4 font-extrabold text-[#39779e]">+880</span><input id="guardian-phone" className="min-w-0 flex-1 bg-transparent px-4 py-4 text-base outline-none placeholder:text-[#94aabd]" value={phone} onChange={(event) => onPhoneChange(event.target.value)} placeholder="01712345678" inputMode="numeric" autoComplete="tel" /></span></label><button type="button" className={`${button} mt-6 w-full bg-[#167ddd] text-white shadow-[0_9px_18px_rgba(22,125,221,.2)] hover:-translate-y-0.5 hover:bg-[#0b6db0]`} disabled={pending} onClick={onContinue}>{pending && <Loader2 className="animate-spin" size={18} />} Continue securely <ArrowRight size={18} /></button></div>;
}
...
export function AccountStage(props: GuardianAccountStageProps) {
  const accountInputClass = "mt-2 w-full border-0 border-b border-[#b7c6d1] bg-transparent px-0 pb-2 text-sm text-[#183d60] outline-none transition placeholder:text-[#a4afba] focus:border-[#167ddd] focus:ring-0";
...
export function RequestStage(props: RequestStageProps) {
  ...
  return <form className="mt-8" onSubmit={props.onSubmit}>
    ...
    <ol className="mt-7 grid gap-2 sm:grid-cols-3" aria-label="Tutor request details progress">{requestSteps.map((label, index) => {
      ...
      return <li key={label} aria-current={isActive ? "step" : undefined} className={`flex min-h-14 items-center gap-3 rounded-2xl border px-3.5 py-3 text-left text-xs font-extrabold transition sm:justify-center ${isActive ? "border-[#8bc8ee] bg-[#e5f5ff] text-[#126ea9] shadow-[0_5px_16px_rgba(22,125,221,.08)]" : isComplete ? "border-[#cce9d7] bg-[#edf9f2] text-[#21824f]" : "border-[#e0eaf0] bg-[#f6f9fb] text-[#7890a1]"}`}><span className={`grid h-6 w-6 shrink-0 place-items-center rounded-full text-[11px] ${isActive ? "bg-[#167ddd] text-white" : isComplete ? "bg-[#21824f] text-white" : "bg-white text-[#7890a1]"}`}>{isComplete ? <Check size={13} aria-hidden="true" /> : index + 1}</span><span>{label}</span></li>;
    })}</ol>
    {props.step === 1 ? <div className="mt-7 space-y-5"><fieldset className="rounded-2xl border border-[#dceaf2] bg-[#fbfdff] p-4 shadow-[0_8px_24px_rgba(28,101,148,.04)] sm:p-5"><legend className="px-1 text-sm font-extrabold text-[#234b6d]">Learning details</legend><p className="mt-1 text-xs leading-5 text-[#71889b]">Start with the student’s curriculum and current level.</p><div className="mt-5 grid gap-5 md:grid-cols-2"><SelectField label="Curriculum / category" value={input.category} onChange={props.onSetCategory} options={categories} placeholder="Choose a category" />{input.category === "English Medium" ? <SelectField label="Curriculum Type" value={input.curriculumType} onChange={props.onSetCurriculumType} options={getGuardianCurriculumTypesForCategory(input.category)} placeholder="Choose a Curriculum Type" /> : null}<SelectField label="Class / level" value={input.classCourse} onChange={props.onSetClassCourse} options={availableLevels} placeholder={input.category ? "Choose a level" : "Choose a curriculum first"} /><InputField label="Student first name" optional helper="Optional — visible only to your coordinator" value={props.studentName} onChange={props.onSetStudentName} maxLength={80} /><SelectField label="Student gender" optional helper="Optional — shown on the Job Board only when selected" value={input.studentGender} onChange={(value) => props.onSetStudentGender(value as StudentGender)} options={["female", "male"]} placeholder="No selection" formatOption={formatStudentGender} /></div><label className="mt-5 block text-sm font-extrabold text-[#315b79]" htmlFor="address-details">Address Details <span className="font-normal text-[#71889b]">(optional)</span><span id="address-details-hint" className="mt-1 block text-xs font-normal leading-5 text-[#71889b]">A landmark or access note for you, the coordinator, and your formally assigned Tutor only. It is never shown on the Job Board.</span><textarea id="address-details" className="form-input mt-2 min-h-24" value={input.addressDetails} onChange={(event) => props.onSetAddressDetails(event.target.value)} maxLength={160} aria-describedby="address-details-hint" /></label></fieldset><fieldset aria-label="Subject selection" className="rounded-2xl border border-[#cfe6f4] bg-[#f5fbff] p-4 shadow-[0_8px_24px_rgba(28,101,148,.04)] sm:p-5"><legend className="px-1 text-sm font-extrabold text-[#234b6d]">Subject selection <span className="text-[#d74545]">*</span></legend><div className="mt-1 flex flex-wrap items-start justify-between gap-3"><p className="max-w-md text-xs leading-5 text-[#617e96]">Choose every subject for which you need a Tutor. You can select more than one.</p><span role="status" aria-live="polite" aria-label={`${input.selectedSubjects.length} subject${input.selectedSubjects.length === 1 ? "" : "s"} selected`} className={`rounded-full px-3 py-1.5 text-xs font-extrabold ${input.selectedSubjects.length ? "bg-[#dff3ff] text-[#126ea9]" : "bg-white text-[#71889b]"}`}>{input.selectedSubjects.length} subject{input.selectedSubjects.length === 1 ? "" : "s"} selected</span></div><div className="mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">{availableSubjects.map((subject) => <ChoiceButton key={subject} selected={input.selectedSubjects.includes(subject)} onClick={() => props.onToggleSubject(subject)}>{subject}</ChoiceButton>)}</div></fieldset></div> : null}
    {props.step === 2 ? <div className="mt-7 space-y-6">
      <fieldset><legend className="text-sm font-extrabold text-[#315b79]">Tuition type <span className="text-[#d74545]">*</span></legend><p className="mt-1 text-xs leading-5 text-[#71889b]">City and location are required for Home, Group, and Package Tutoring.</p><div className="mt-3 flex flex-wrap gap-2">{(["home", "online", "group", "package"] as const).map((value) => <ChoiceButton key={value} selected={input.tuitionType === value} onClick={() => props.onSetTuitionType(value)}>{formatTuitionType(value)}</ChoiceButton>)}</div></fieldset>
      {input.tuitionType === "home" || input.tuitionType === "online" || input.tuitionType === "package" ? <label className="block max-w-sm text-sm font-extrabold text-[#315b79]" htmlFor="student-count">Number of students <span className="text-[#d74545]">*</span><span id="student-count-hint" className="mt-1 block text-xs font-normal leading-5 text-[#71889b]">Enter the number of students for this request (1–100).</span><input id="student-count" className="form-input mt-2" type="number" min={1} max={100} step={1} inputMode="numeric" aria-describedby="student-count-hint" value={input.studentCount} onChange={(event) => props.onSetStudentCount(event.target.value)} /></label> : null}
      {input.tuitionType !== "online" ? <><div className="grid gap-5 md:grid-cols-2"><SearchableLocationSelect label="Tuition City" value={input.tuitionCityLocationId} options={props.cities} placeholder="Search a City" searchPlaceholder="Search City" emptyMessage="No City matches your search." required onChange={props.onSetTuitionCity} /><SearchableLocationSelect label="Thana / Upazila / Area / Sub-area" value={input.tuitionLocationId} options={props.tuitionLocations} placeholder="Choose a City first" searchPlaceholder="Search location or Sub-area" emptyMessage="No location matches your search." disabled={!input.tuitionCityLocationId} required countContext={props.tuitionCityLabel} onChange={props.onSetTuitionLocation} /></div>{locationSelection ? <div role="status" aria-label="Location selected" className="mt-4 flex flex-wrap items-start gap-3 rounded-xl border border-[#bfe5cf] bg-[#f1fbf5] px-4 py-3 text-sm leading-6 text-[#286b49]"><Check className="mt-1 shrink-0 text-[#188f73]" size={17} aria-hidden="true" /><p className="min-w-0 flex-1"><span className="font-extrabold">Location selected.</span> {locationSelection.cityLabel} — {locationSelection.locationLabel}</p><button type="button" className="min-h-11 shrink-0 rounded-lg px-2.5 py-1.5 text-xs font-extrabold text-[#2377bd] underline decoration-[#8bc8a5] underline-offset-4 transition hover:bg-white/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#167ddd] focus-visible:ring-offset-2" aria-label="Change selected location" onClick={() => props.onSetTuitionLocation("")}>Change location</button></div> : null}</> : <p className="rounded-xl border border-[#d7e9f2] bg-[#f4faff] px-4 py-3 text-sm leading-6 text-[#58758a]">Exact physical location is not needed for Online Tutoring.</p>}
      {input.tuitionType === "group" ? <label className="block max-w-sm text-sm font-extrabold text-[#315b79]" htmlFor="group-capacity">Maximum students <span className="text-[#d74545]">*</span><span id="group-capacity-hint" className="mt-1 block text-xs font-normal leading-5 text-[#71889b]">Enter the maximum number of students in this Group Tutoring request (2–100).</span><input id="group-capacity" className="form-input mt-2" type="number" min={2} max={100} step={1} inputMode="numeric" aria-describedby="group-capacity-hint" value={input.groupCapacity} onChange={(event) => props.onSetGroupCapacity(event.target.value)} /></label> : null}
      {input.tuitionType === "package" ? <label className="block max-w-sm text-sm font-extrabold text-[#315b79]" htmlFor="package-duration-months">Package duration (months) <span className="text-[#d74545]">*</span><span id="package-duration-months-hint" className="mt-1 block text-xs font-normal leading-5 text-[#71889b]">Enter the full Package Tutoring duration in months (1–24).</span><input id="package-duration-months" className="form-input mt-2" type="number" min={1} max={24} step={1} inputMode="numeric" aria-describedby="package-duration-months-hint" value={input.packageDurationMonths} onChange={(event) => props.onSetPackageDurationMonths(event.target.value)} /></label> : null}
      <div className="grid gap-5 md:grid-cols-2"><SelectField label="Days per week" value={input.daysPerWeek} onChange={props.onSetDays} options={["1", "2", "3", "4", "5", "6", "7"]} placeholder="Choose days" formatOption={(value) => `${value} day${value === "1" ? "" : "s"}`} /><SelectField label="Preferred Tutor gender" value={input.preferredGender} onChange={(value) => props.onSetPreferredGender(value as PreferredGender)} options={["any", "female", "male"]} placeholder="Choose a preference" formatOption={formatPreferredGender} /></div>
      <fieldset><legend className="text-sm font-extrabold text-[#315b79]">Monthly budget <span className="text-[#d74545]">*</span></legend><div className="mt-3 flex flex-wrap gap-2"><ChoiceButton selected={input.budgetKind === "range"} onClick={() => props.onSetBudgetKind("range")}>Budget range</ChoiceButton><ChoiceButton selected={input.budgetKind === "discuss"} onClick={() => props.onSetBudgetKind("discuss")}>Discuss with coordinator</ChoiceButton></div>{input.budgetKind === "range" ? <div className="mt-4 grid gap-4 sm:grid-cols-2"><InputField label="Minimum (BDT)" value={input.budgetMinimum} onChange={props.onSetBudgetMinimum} inputMode="numeric" /><InputField label="Maximum (BDT)" value={input.budgetMaximum} onChange={props.onSetBudgetMaximum} inputMode="numeric" /></div> : null}</fieldset>
      <label className="block text-sm font-extrabold text-[#315b79]">Additional notes <span className="font-normal text-[#71889b]">(optional)</span><textarea className="form-input mt-2 min-h-28" value={props.notes} onChange={(event) => props.onSetNotes(event.target.value)} maxLength={2000} /></label>
    </div> : null}
    ...
    <div className="mt-8 flex flex-col-reverse justify-between gap-3 border-t border-[#e6eef4] pt-6 sm:flex-row sm:items-center">{props.step > 1 ? <button type="button" className={`${button} bg-[#eef5f9] text-[#365d7d] hover:bg-[#e1eff7]`} onClick={props.onBack}><ArrowLeft size={17} /> Back</button> : <span className="hidden sm:block" />}{props.step < 3 ? <button type="button" className={`${button} w-full bg-[#167ddd] text-white shadow-[0_9px_18px_rgba(22,125,221,.2)] hover:-translate-y-0.5 hover:bg-[#0b6db0] sm:w-auto`} aria-label="Continue to tuition preferences" onClick={props.onAdvance}>Continue <ArrowRight size={17} /></button> : <button type="submit" className={`${button} w-full bg-[#167ddd] text-white shadow-[0_9px_18px_rgba(22,125,221,.2)] hover:-translate-y-0.5 hover:bg-[#0b6db0] sm:w-auto`} disabled={props.pending} aria-label={props.pending ? "Sending request" : "Send request"}>{props.pending && <Loader2 className="animate-spin" size={18} />}{props.pending ? "Sending request" : "Send request"} <ArrowRight size={17} /></button>}</div>
  </form>;
}
...
function RequestPreview({ input, studentName, notes, tuitionCityLabel, tuitionLocationLabel, onEditStep }: { input: RequestInput; studentName: string; notes: string; tuitionCityLabel: string; tuitionLocationLabel: string; onEditStep: (step: 1 | 2) => void }) {
  ...
  return <div className="mt-7 space-y-5"><div className="rounded-2xl border border-[#d7e9f2] bg-[#f4faff] p-5 text-sm leading-7 text-[#385e79]"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-extrabold text-[#173b60]">Review your request</h3><p className="mt-1 text-xs leading-5 text-[#607d92]">Check the details below. You can return to either section without losing your entries.</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-extrabold text-[#2a739f]">Private review</span></div><section className="mt-5 rounded-xl border border-[#dceaf2] bg-white p-4" aria-labelledby="review-learning-needs"><div className="flex flex-wrap items-center justify-between gap-3"><h4 id="review-learning-needs" className="font-extrabold text-[#234b6d]">Learning needs</h4><button type="button" className="min-h-11 rounded-lg px-3 text-xs font-extrabold text-[#167ddd] underline underline-offset-4 focus:outline-none focus:ring-2 focus:ring-[#167ddd]/30" onClick={() => onEditStep(1)}>Edit learning needs</button></div><dl className="mt-3 grid gap-x-6 gap-y-2 sm:grid-cols-2"><ReviewItem label="Category" value={input.category} />{input.curriculumType ? <ReviewItem label="Curriculum Type" value={input.curriculumType} /> : null}<ReviewItem label="Class / level" value={input.classCourse} /><ReviewItem label="Subjects" value={input.selectedSubjects.join(", ")} />{studentName ? <ReviewItem label="Student first name" value={studentName} /> : null}{input.studentGender ? <ReviewItem label="Student gender" value={formatStudentGender(input.studentGender)} /> : null}{input.addressDetails ? <ReviewItem label="Address Details" value={input.addressDetails} /> : null}</dl></section><section className="mt-4 rounded-xl border border-[#dceaf2] bg-white p-4" aria-labelledby="review-tuition-preferences"><div className="flex flex-wrap items-center justify-between gap-3"><h4 id="review-tuition-preferences" className="font-extrabold text-[#234b6d]">Tuition preferences</h4><button type="button" className="min-h-11 rounded-lg px-3 text-xs font-extrabold text-[#167ddd] underline underline-offset-4 focus:outline-none focus:ring-2 focus:ring-[#167ddd]/30" onClick={() => onEditStep(2)}>Edit tuition preferences</button></div><dl className="mt-3 grid gap-x-6 gap-y-2 sm:grid-cols-2"><ReviewItem label="Tuition type" value={formatTuitionType(input.tuitionType)} />{input.tuitionType === "group" ? <ReviewItem label="Maximum students" value={`${input.groupCapacity} students`} /> : null}{input.tuitionType === "home" || input.tuitionType === "online" || input.tuitionType === "package" ? <ReviewItem label="Number of students" value={`${input.studentCount} student${input.studentCount === "1" ? "" : "s"}`} /> : null}{input.tuitionType === "package" ? <ReviewItem label="Package duration" value={`${input.packageDurationMonths} month${input.packageDurationMonths === "1" ? "" : "s"}`} /> : null}<ReviewItem label="Location" value={location} /><ReviewItem label="Days per week" value={`${input.daysPerWeek} day${input.daysPerWeek === "1" ? "" : "s"}`} /><ReviewItem label="Preferred Tutor gender" value={formatPreferredGender(input.preferredGender)} /><ReviewItem label="Budget" value={formatBudget(input)} />{notes ? <ReviewItem label="Additional notes" value={notes} /> : null}</dl></section></div><p className="rounded-xl border border-[#ead8a8] bg-[#fffbf0] px-4 py-3 text-sm leading-6 text-[#6f5b2a]">After you send this request, a coordinator will review it and will confirm any change with you before a job is published. Contact details and exact addresses stay private.</p></div>;
}

export function SuccessState({ requestId }: { requestId: number | null }) {
  return <div className="py-8 text-center sm:py-12"><div className="mx-auto grid size-20 place-items-center rounded-[1.7rem] bg-gradient-to-br from-[#dff6e8] to-[#edfaff] text-[#188f73] shadow-[0_14px_34px_rgba(24,143,115,.14)]"><Check size={38} strokeWidth={2.7} /></div><p className="mt-7 text-xs font-extrabold uppercase tracking-[.18em] text-[#1680bf]">Request received securely</p><h2 className="mx-auto mt-3 max-w-2xl text-3xl font-extrabold tracking-[-.045em] text-[#173b60] sm:text-4xl">Thank you. Your request is now pending review.</h2><p className="mx-auto mt-4 max-w-xl leading-7 text-[#58758a]">Your Request ID is <strong className="text-[#173b60]">#{requestId}</strong>. A coordinator will review the learning needs and contact you to confirm any changes before any job is published.</p><div className="mx-auto mt-6 max-w-xl rounded-2xl border border-[#d8e9f4] bg-[#f6fbff] px-5 py-4 text-left text-sm leading-6 text-[#526f87]"><p className="font-extrabold text-[#234b6d]">Your privacy remains protected</p><p className="mt-1">Your phone, email, exact address, student identity, and additional notes are not shown on the Job Board.</p></div><div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><Link href="/guardian/dashboard/posted-jobs" className={`${button} bg-[#167ddd] text-white shadow-[0_9px_18px_rgba(22,125,221,.2)]`}>View My Requests <ArrowRight size={18} /></Link><Link href="/guardian/dashboard/hire" className={`${button} bg-[#eef5f9] text-[#365d7d]`}>Post Another Request</Link></div></div>;
}

function Eyebrow({ step, title, copy }: { step: string; title: string; copy: string }) { return <div className="border-b border-[#e6eef4] pb-5"><p className="text-xs font-extrabold uppercase tracking-[.16em] text-[#df9a1d]">{step}</p><h2 className="mt-2 text-2xl font-extrabold tracking-[-.045em] text-[#173d60]">{title}</h2><p className="mt-2 text-sm leading-6 text-[#617e96]">{copy}</p></div>; }
