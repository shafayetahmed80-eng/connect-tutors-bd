...
function AccessDenied() {
  return <section className="mx-auto flex min-h-[50vh] max-w-xl flex-col items-center justify-center rounded-3xl border border-j-border bg-white p-8 text-center shadow-sm"><ShieldCheck className="mb-4 h-12 w-12 text-j-ink-faint" /><h1 className="text-2xl font-bold text-j-ink">Owner access required</h1><p className="mt-2 text-sm leading-6 text-j-ink-soft">Only the Project Owner can manage Admin credentials and security records.</p></section>;
}
...
export function PasswordPolicyBanner({ password, confirmPassword }: { password: string; confirmPassword: string }) {
  ...
  const strengthIndicator = !password
    ? { label: "Start typing", value: 0, textClass: "text-j-ink-soft", fillClass: "bg-j-ink-faint" }
  ...
  return <section aria-labelledby="admin-password-policy-title" className="rounded-xl border border-sky-200 bg-white p-3 text-xs text-j-ink-soft">
    <h3 id="admin-password-policy-title" className="font-bold text-j-ink">{adminPasswordPolicy.title}</h3>
    <ul className="mt-2 list-disc space-y-1 pl-4">
      {adminPasswordPolicy.required.map(item => <li key={item}>{item}</li>)}
    </ul>
    <p className="mt-2 leading-5 text-j-ink-soft">{adminPasswordPolicy.recommended}</p>
  ...
  return <>
    ...
    <div>
      ...
      <div className="relative mt-1">
        ...
        <button type="button" aria-controls="admin-confirm-new-password" aria-label={isConfirmationVisible ? "Hide confirm new password" : "Show confirm new password"} aria-pressed={isConfirmationVisible} onClick={() => setIsConfirmationVisible(visible => !visible)} className="absolute inset-y-0 right-1 inline-flex w-9 items-center justify-center rounded-md text-j-ink-soft outline-none hover:bg-j-surface-muted hover:text-j-ink focus-visible:ring-2 focus-visible:ring-j-accent focus-visible:ring-offset-1">
...
function SecurityWorkspaceContent() {
  ...
  if (loading || workspaceAccess.isLoading) return <div className="flex min-h-[60vh] items-center justify-center text-j-ink-soft"><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Checking Owner permissions…</div>;
