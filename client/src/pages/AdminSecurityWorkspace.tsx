...
function SecurityWorkspaceContent() {
  ...
  const auditColumns: RecordColumn<AuditEntry>[] = [
    { key: "time", label: "Time", place: "head", cellClassName: "whitespace-nowrap", cell: entry => new Date(entry.createdAt).toLocaleString() },
    { key: "event", label: "Event", place: "head", cell: entry => <span className="font-semibold capitalize">{entry.event.replaceAll("_", " ")}</span> },
    { key: "account", label: "Account", cell: entry => entry.email ?? "—" },
    { key: "context", label: "Context", wide: true, cell: entry => <span className="text-xs text-j-ink-muted">{formatAuditMetadata(entry.metadata)}</span> },
  ];

  type AuthEntry = NonNullable<typeof authEvents.data>["items"][number];
  const authColumns: RecordColumn<AuthEntry>[] = [
    { key: "time", label: "Time", place: "head", cellClassName: "whitespace-nowrap", cell: entry => new Date(entry.createdAt).toLocaleString() },
    { key: "event", label: "Event", place: "head", cell: entry => <span className="font-semibold capitalize">{entry.event.replaceAll("_", " ")}</span> },
    { key: "role", label: "Role", cell: entry => <span className="capitalize">{entry.role ?? "—"}</span> },
    { key: "identifier", label: "Identifier", cell: entry => <span className="font-mono text-xs">{entry.identifierMasked ?? "—"}</span> },
    { key: "ip", label: "IP", cell: entry => <span className="font-mono text-xs">{entry.ip ?? "—"}</span> },
  ...
  return <div className="mx-auto max-w-7xl space-y-5 pb-10">
    ...
    <section className="rounded-xl border border-j-border bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-lg font-bold text-j-ink">Security audit log</h2></div><div className="flex flex-col gap-3 sm:flex-row sm:items-end"><label className="text-sm font-semibold text-j-ink-strong">Account email<input type="search" value={auditEmail} onChange={e => { setAuditEmail(e.target.value); setPage(1); }} placeholder="Filter by email" className="mt-2 block h-10 rounded-xl border border-j-field-border px-3 text-sm font-normal outline-none focus:border-j-accent" /></label><label className="text-sm font-semibold text-j-ink-strong">Event<select value={event} onChange={e => { setEvent(e.target.value as typeof event); setPage(1); }} className="mt-2 block h-10 rounded-xl border border-j-field-border bg-white px-3 text-sm font-normal outline-none focus:border-j-accent">{events.map(option => <option key={option} value={option}>{option.replaceAll("_", " ")}</option>)}</select></label></div></div>{audit.isLoading ? <div className="mt-5 flex items-center text-sm text-j-ink-soft"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Loading security events…</div> : audit.isError ? <InlineError message="The audit log could not be loaded." /> : <><div className="mt-5 overflow-x-auto"><RecordTable
        plain
        caption="Authentication events: when, what, which role, and why"
        columns={authColumns}
