...
describe("Tutor Apply Now return routing", () => {
  it("keeps an allowlisted Job Board detail as a Tutor sign-in return destination", () => {
    const returnPath = buildTutorApplyReturnPath("CT-JOB-000042");

    expect(returnPath).toBe("/job-board?job=CT-JOB-000042");
    expect(buildTutorApplySignInPath("CT-JOB-000042")).toBe(
      "/auth?role=tutor&returnTo=%2Fjob-board%3Fjob%3DCT-JOB-000042",
    );
  ...
  it("rejects external, privileged, malformed, and multi-parameter return destinations", () => {
    expect(getSafeTutorApplyReturnPath("https://example.com/job-board?job=CT-JOB-000042")).toBeNull();
    expect(getSafeTutorApplyReturnPath("/admin/matching?job=CT-JOB-000042")).toBeNull();
    expect(getSafeTutorApplyReturnPath("/job-board?job=CT-JOB-000042&role=tutor")).toBeNull();
  ...
  it("restores the selected job only from the public board or allowlisted Tutor panel locations", () => {
    ...
    expect(getTutorApplyReturnFromLocation("/tutor/dashboard/jobs?returnTo=%2Fjob-board%3Fjob%3DCT-JOB-000042&role=tutor")).toBeNull();
  });
});
